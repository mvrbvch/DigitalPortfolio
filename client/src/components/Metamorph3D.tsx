  ...
  useEffect(() => {
    ...
    const applyNoiseDeformation = (time: number, morphState: number) => {
      ...
      for (let i = 0; i < positions.length; i += 3) {
        ...
        const vertexTime = time + originalX * 10 + originalY * 10 + originalZ * 10;
        ...
        if (variant === 'torus') {
          ...
          noiseX = Math.sin(vertexTime * 0.2) * Math.cos(vertexTime * 0.1) * 0.15;
        ...
        positions[i] = originalX + noiseX * noiseIntensityRef.current * morphState;
        positions[i+1] = originalY + noiseY * noiseIntensityRef.current * morphState;
        positions[i+2] = originalZ + noiseZ * noiseIntensityRef.current * morphState;
      }
    ...
    // Animation loop
    const animate = () => {
      if (!sceneRef.current || !cameraRef.current || !rendererRef.current || !objectRef.current) return;
